...
from server.realtime.speculation import SpeculationSlot
...
class ReplyController:
    def __init__(self, send_json, send_bytes, settings: Settings, state,
        ...
        self._speculation = SpeculationSlot(self._supervisor)
        ...
        # playback (extraction.py): it runs in turn order and never gates speech.
        self._extraction = ExtractionQueue(self._engine, self._send, self.interview)
    ...
    async def on_turn(self, turn: TurnComplete, turn_id: int) -> None:
        spec = self._speculation.take_if_matches(turn_id, turn.transcript)
        if spec is not None:
            # PROMOTE: the guessed turn is the committed turn. The generation
            # has been running since the guess; fill the timing anchors and
            ...
            self._extraction.queue(token, turn.transcript)
            spec["release"].set()
            return
    ...
    async def on_chat(self, text: str, turn_id: int) -> None:
        ...
        self._extraction.queue(token, text)
    ...
    async def speculate(self, transcript: str, turn_id: int) -> None:
        ...
        (speculation.py holds the safety argument)."""
        if self._speaker is None or not isinstance(self._engine, LlmEngine):
            return
        await self._speculation.start(transcript, turn_id, self._deliver_draft)

    async def _deliver_draft(self, owned: GenerationToken, transcript: str,
                             anchors: dict, release: asyncio.Event) -> None:
        await self._deliver_voice(
            owned, self._voiced_sentences(owned, transcript, commit_gate=release),
            anchors, record_metrics=True, release=release)

    async def cancel_speculation(self) -> None:
        await self._speculation.cancel()

    async def _replace_current(self) -> None:
        if self._speculation.current is not None:
